...
const VERSION_MESSAGE: &[u8] = &[
    ...
    0x67, 0x00, 0x00, 0x00, 0x38, 0x7f, 0x3c, 0xa1, 0x80, 0x11, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x69, 0x6e, 0x65, 0x3a, 0x31, 0x2e, 0x30, 0x2e, 0x30, 0x2f, 0x00, 0x00, 0x00, 0x00, 0x00,
];
...
mod tests {
    ...
    #[test]
    fn version_message_identifies_the_current_bitengine_release() -> Result<()> {
        let payload = VERSION_MESSAGE
            .get(24..)
            .context("test version message payload")?;
        let expected_user_agent = format!("/BitEngine:{}/", env!("CARGO_PKG_VERSION"));
        assert!(payload
            .windows(expected_user_agent.len())
            .any(|window| window == expected_user_agent.as_bytes()));
        let declared_checksum: [u8; 4] = VERSION_MESSAGE
            .get(20..24)
