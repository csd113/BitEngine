...
impl<R: Runtime, M: Mockable<R>> IptManager<R, M> {
    ...
    fn expire_old_expiry_times(&self, publish_set: &mut PublishIptSet, now: &TrackingNow) {
        ...
        retain_unexpired_publication_records(
            &mut publish_set.last_descriptor_expiry_including_slop,
            now,
        );
    }
...
/// Discard publication records once their descriptors and allowance have expired.
fn retain_unexpired_publication_records(records: &mut HashMap<IptLocalId, Instant>, now: Instant) {
    records.retain(|_lid, expiry| *expiry > now);
}
...
mod test {
    ...
    #[test]
    fn publication_expiry_retains_only_still_valid_records() {
        let now = Instant::now();
        let past = now.checked_sub(Duration::from_secs(1)).unwrap();
        let future = now.checked_add(Duration::from_secs(1)).unwrap();
        let past_lid = IptLocalId::dummy(1);
        let boundary_lid = IptLocalId::dummy(2);
        let future_lid = IptLocalId::dummy(3);
        let mut records =
            HashMap::from([(past_lid, past), (boundary_lid, now), (future_lid, future)]);

        retain_unexpired_publication_records(&mut records, now);

        assert_eq!(records, HashMap::from([(future_lid, future)]));
    }
